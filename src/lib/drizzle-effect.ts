...
type PropertySignatureEncoded<T> = T extends Schema.PropertySignature<any, any, any, any, infer From, any, any>
	? From
	: never

type PropertySignatureType<T> = T extends Schema.PropertySignature<any, infer To, any, any, any, any, any> ? To : never
	...
	| ((
	  ...
	  >
			? Schema.Schema<
				...
				>
			: Schema.Schema<
					Exclude<Schema.Schema.Encoded<InsertColumnPropertySignatures<TTable>[Col]>, undefined | null>,
					any
				>)
...
export const Json = Schema.suspend(
	(): Schema.Schema<Json> =>
		Schema.Union(literalSchema, Schema.Array(Json), Schema.Record({ key: Schema.String, value: Json })),
)
...
type BuildInsertSchema<TTable extends Drizzle.Table, TRefine extends InsertRefine<TTable> | {} = {}> = Schema.Struct<
	InsertColumnPropertySignatures<TTable> & {
		[K in keyof TRefine & string]: InsertColumnPropertySignatures<TTable>[K] extends Schema.PropertySignature<
			any,
			any,
			any,
			any,
...
type BuildSelectSchema<TTable extends Drizzle.Table, TRefine extends InsertRefine<TTable> | {} = {}> = Schema.Struct<
	{
		[K in keyof SelectColumnPropertySignatures<TTable>]: SelectColumnPropertySignatures<TTable>[K]
	} & {
		[K in keyof TRefine & string]: SelectColumnPropertySignatures<TTable>[K] extends Schema.PropertySignature<
			any,
			any,
			any,
			any,
